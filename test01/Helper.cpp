...
static uintptr_t** g_deviceFunctionAddresses;
...
uintptr_t* GetD3D9DeviceFunctionAddress(short methodIndex)
{
	...
	if (!g_deviceFunctionAddresses) {
		...
		HWND hWnd = CreateWindowA("BUTTON", "Temporary Window", WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 300, 300, NULL, NULL, NULL, NULL);
		__try 
		{
			...
			if(pD3D == NULL )
			{
				...
				add_log("GetD3D9DeviceFunctionAddress: Direct3DCreate9 failed");
				return 0;
			}
			__try 
			{
												  ...
												  dwBehaviorFlags, &d3dpp, &pd3dDevice ) ) )
				{
					...
					add_log("GetD3D9DeviceFunctionAddress: IDirect3D9::CreateDevice failed");
					return 0;
				}
...
{
	if(s_bFailed == false && ofile.is_open() == false) {
#if 0
		GetModuleFileNameA(NULL, dlldir, 512);
		for(int i = strlen(dlldir); i > 0; i--) { if(dlldir[i] == '\\') { dlldir[i+1] = 0; break; } }
		ofile.open(GetDirectoryFile("injector_log.txt"), ios::app);
#else
		ofile.open("C:\\Log\\injector_log.txt", ios::out | ios::trunc);
	...
	if(s_bFailed == false && ofile.is_open() == true)
	{
		if(!fmt) { return; }
